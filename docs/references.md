...
| Workspace | `GET /api/workspace?sections=catalog,inbox,memberships,project,summaries,todos&projectId=:id` (`project`/`todos` require `projectId`; no sections preserves the complete legacy response), `GET /api/workspace/search?organizationId=:id|personal&q=:query` returning authorized project IDs, `GET /api/todos/:todoId/workspace` for an authorized deep-link project snapshot, `GET /api/my-work?organizationId=:id|personal`, `GET /api/notifications`, notification read/dismiss routes, `GET/PUT /api/notification-subscription` |
