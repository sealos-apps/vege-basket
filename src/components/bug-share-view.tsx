  ...
  return <main className="bug-share-screen">
    <div className="bug-share-shell">
      ...
      {data ? <>
        ...
        <div className="bug-share-meta"><span>测试用例：{data.testCaseId ? `CASE-${data.testCaseId} ${data.testCaseTitle || ''}` : '待补关联'}</span><span>用例目录：{data.testCaseId ? data.testCaseFolderName || '未分类' : '待补关联'}</span><span>模块：{data.moduleName || '无模块'}</span>{data.testPlanName ? <span>测试计划：{data.testPlanName}</span> : null}{data.assigneeName ? <span>负责人：<UserName departedUserIds={data.departedUserIds} name={data.assigneeName} userId={data.assigneeUserId} /></span> : null}<span>更新时间：{new Date(data.updatedAt).toLocaleString()}</span></div>
