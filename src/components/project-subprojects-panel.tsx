...
export function ProjectSubprojectsPanel({ projectId, canManage, onChange }: { projectId: number; canManage: boolean; onChange?: (data: WorkspaceData) => void }) {
  ...
  async function mutate(operation: () => Promise<WorkspaceData>) {
    ...
    {items.map(item => <div className="organization-project-detail-heading" key={item.id}>
      ...
      <div><span>{item.taskCount ?? 0} 个任务</span>
        {canManage && <>
          ...
          <Button type="button" variant="ghost" size="icon" disabled={busy || Boolean(item.taskCount)} title={item.taskCount ? '已有任务引用，不能删除' : '删除子项目'} aria-label={`删除${item.name}`} onClick={() => { if (window.confirm(`删除子项目“${item.name}”？`)) void mutate(() => removeProjectSubproject(projectId, item.id)) }}><Trash size={16} /></Button>
        </>}
      </div>
    </div>)}
