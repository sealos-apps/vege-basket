...
const verificationBranchMigrationSource = readFileSync(new URL('./migrations/20260917_test_bug_verification_package_branches.sql', import.meta.url), 'utf8')
...
test('case workbench loads the whole space and treats test subjects as virtual root directories', () => {
  ...
  assert.match(testWorkbenchClientSource, /importTestCases\(spaceId, csvText\)/u)
  assert.doesNotMatch(testWorkbenchClientSource, /导入方式/u)
  assert.match(testWorkbenchClientSource, /moduleExamples\.join\('、'\)/u)
  ...
  assert.match(testCaseDirectoryTreeSource, /const selectedRoot = selected === 'all'[\s\S]*\.directoryRoot/u)
  assert.match(testCaseDirectoryTreeSource, /selectedRoot \? '新增一级目录' : '新增子目录'/u)
...
test('Bug scope stays within the current space and exposes optional case and module fields', () => {
  ...
  assert.match(testWorkbenchSource, /b\.organization_module_id/u)
  assert.match(testWorkbenchSource, /moduleName: row\.organization_module_id \? modulesById\.get\(Number\(row\.organization_module_id\)\)\?\.name : undefined/u)
...
test('verification submissions snapshot one delivery mode and emit an immutable acceptance comment atomically', () => {
  ...
  assert.match(schemaSource, /add column if not exists ci_branch text/u)
  assert.match(schemaSource, /test_bug_verification_packages_ci_branch_check/u)
  ...
  assert.match(verificationBranchMigrationSource, /^begin;$/mu)
  assert.match(verificationBranchMigrationSource, /split_part\(split_part\(object_key, '\/ci\/', 2\), '\/', 1\)/u)
  assert.match(verificationBranchMigrationSource, /test_bug_verification_packages_ci_branch_check/u)
  assert.match(verificationBranchMigrationSource, /^commit;$/mu)
  ...
  assert.match(testWorkbenchSource, /requestedCiBranch \|\| objectCiBranch/u)
  assert.match(testWorkbenchSource, /objectCiBranch !== requestedCiBranch/u)
  ...
  assert.match(testWorkbenchClientSource, /item\.channel === 'ci' && item\.ciBranch/u)
})
...
test('verification package picker supports filtered paginated catalogs and incremental version loading', () => {
  ...
  assert.match(testWorkbenchClientSource, /fetchPackageMarketCiBranches/u)
  assert.match(testWorkbenchClientSource, /ciBranch: ciBranch \|\| undefined/u)
  assert.match(testWorkbenchClientSource, /disabled=\{Boolean\(selectedRulePackage\)\}/u)
  assert.match(testWorkbenchClientSource, /selectedRulePackage\.arch !== arch/u)
  assert.match(testWorkbenchClientSource, /ciBranch: selectedBranch \|\| undefined/u)
...
test('test subject editing uses a dedicated patch route without version or environment fields', () => {
  ...
  assert.match(patchRoute, /只有一级目录创建者可以编辑/u)
