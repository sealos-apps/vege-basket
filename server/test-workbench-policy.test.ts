...
test('developer workbench offers start and reject for pending confirmation Bugs', () => {
  ...
  assert.match(testWorkbenchClientSource, /title=\{`驳回 Bug/u)
...
test('test case deletion is exposed only when allowed and requires confirmation', () => {
  ...
  assert.match(testWorkbenchClientSource, /title="删除测试用例"/u)
  assert.match(testWorkbenchClientSource, /已加入计划的执行快照继续保留/u)
  assert.match(testWorkbenchClientSource, /deleteTestCase\(casePendingDelete\.testSpaceId, casePendingDelete\.id\)/u)
  assert.match(testWorkbenchClientSource, /<ConfirmActionDialog[\s\S]*?open=\{caseDeleteDialogOpen\}[\s\S]*?onOpenChange=\{setCaseDeleteDialogOpen\}/u)
  ...
  assert.match(testWorkbenchClientSource, /<ConfirmActionDialog[\s\S]*?open=\{planDeleteDialogOpen\}[\s\S]*?onOpenChange=\{setPlanDeleteDialogOpen\}/u)
