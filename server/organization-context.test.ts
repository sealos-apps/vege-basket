...
test('organization management follows role selection in both account menus', () => {
  ...
  assert.match(appSource, /function openOrganizationManagement\(\)[\s\S]+organizationReturnViewRef\.current = view[\s\S]+setView\('organization'\)/u)
  assert.match(appSource, /function closeOrganizationManagement\(\)[\s\S]+canUseViewForUser\(returnView, authUser\)/u)
