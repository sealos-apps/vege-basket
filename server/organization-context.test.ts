...
test('organization management follows role selection in both account menus', () => {
  ...
  assert.match(accountMenuSource, /onSelect=\{activeView === 'organization' \? onCloseOrganization : onOpenOrganization\}/u)
  assert.match(accountMenuSource, /<ArrowLeft \/> 返回工作区/u)
  ...
  assert.match(appSource, /function openOrganizationManagement\(\)/u)
  assert.equal((appSource.match(/onOpenOrganization=\{openOrganizationManagement\}/gu) ?? []).length, 2)
  assert.equal((appSource.match(/onCloseOrganization=\{closeOrganizationManagement\}/gu) ?? []).length, 2)
